    ...
    <div class="py-12 px-4">
        <div class="max-w-3xl mx-auto">
            <div class="mb-8">
                ...
                <p class="text-slate-400 mt-1">Update your article</p>
            </div>
                    ...
                    <div>
                        ...
                        <select name="category_id" id="category_id" required
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" {{ old('category_id', $article->category_id) == $category->id ? 'selected' : '' }} class="text-slate-900">
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <label class="relative flex items-center p-4 border border-white/10 rounded-xl cursor-pointer hover:border-indigo-500/50 transition-colors has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-500/10">
                                <input type="radio" name="status" value="draft" {{ old('status', $article->status) == 'draft' ? 'checked' : '' }} class="sr-only">
                                <div class="flex-1">
                            ...
                            <label class="relative flex items-center p-4 border border-white/10 rounded-xl cursor-pointer hover:border-indigo-500/50 transition-colors has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-500/10">
                                <input type="radio" name="status" value="published" {{ old('status', $article->status) == 'published' ? 'checked' : '' }} class="sr-only">
                                <div class="flex-1">
